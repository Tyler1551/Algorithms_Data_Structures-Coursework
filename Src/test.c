...
//sets all positons to blank in the game board
...
//When called, the player is switched
void switch_char()
{
	switch(input)
	{
...
//Scanf used to set the pos and places the character 'X' or 'O' into the index of the board
void insert(char* game_board)
{
	...
	//if the index isnt equal to a blank space, then we switch character back to what it was for the player to play a different position
	if(game_board[pos] == ' ')
	{
...
*/
	...
	//Series of IF statements to check for winning combos
	if(game_board[7] == 'X' && game_board[8] == 'X' && game_board[9] == 'X')
	{
		gamewon = true;
		printf("\nCrosses win! Press CTRL+C to exit\n");
		return gamewon;
	...
	{
		gamewon = true;
		printf("\nNoughts win! Press CTRL+C to exit\n");
		return gamewon;
